...
def fetch_content_data_from_postgres(account_id: int = ACCOUNT_ID) -> dict[int, dict[str, Any]]:
    """
    Fetch content data from Postgres for creators with the given account_id:
    ...
    query = text(
        ...
        INNER JOIN public.expert_user e
    ...
    try:
        ...
        logger.info(f"Fetched {len(rows)} content records from Postgres")
...
def store_content_in_qdrant(
    ...
    """
    Store/update content data in Qdrant.
    
    Args:
        content_data: {content_id: {"title": ..., "language_code": ...}}
        dry_run: If True, don't actually write to Qdrant
        upsert_mode: If True, update existing points; if False, insert only
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Starting Qdrant storage (dry_run={dry_run}, upsert_mode={upsert_mode})")
    logger.info(f"{'='*60}")
    ...
    # Get all existing points in Qdrant first (for upsert mode)
    ...
    try:
        ...
        for content_id, data in content_data.items():
            try:
                ...
                is_update = content_id in existing_point_ids
                
                if is_update:
                    # Update existing point's payload
                    ...
                    logger.info(f"[UPDATE] content_id={content_id}, title={title}, language={language_code}")
                else:
                    # Insert new point with vector embedding
                    try:
                        ...
                        logger.info(f"[INSERT] content_id={content_id}, title={title[:30]}..., language={language_code}")
                    except Exception as e:
                        failed_count += 1
                        logger.error(f"Failed to create embedding for content_id={content_id}: {e}")
